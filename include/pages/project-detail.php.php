...
elseif(isset($_POST['delete'])){
    echo $project_id = $_POST['project'];

    if(isset($_POST['confirm_delete'])){
        die('ja');
        echo $delete_row = $db->query("DELETE FROM projects WHERE project_id = '".$project_id."'");
        $db->execute();
        $melding = 'De items zijn succesvol verwijderd.';
    //echo '<meta http-equiv="refresh" content="0; url='.$website.'/'.$url1.'">';
    }
    else{
        echo '<p>Weet je zeker dat je deze item(s) wilt verwijderen? <br> Dit zal niet meer ongedaan gemaakt kunnen worden</p>';
        echo '<form method="post">';
            echo '<input type="submit" name="cancel" class="knop" value="Annuleren">';
            echo '<input type="submit" name="confirm_delete" class="delete-knop" value="Verwijderen">';
        echo '</form>';
    }
    ...
    <form method="post" enctype="multipart/form-data">
        <div class="filter-wrap">
            <div class="buttons-wrap"> 
                <input type="submit" name="add" class="nieuw-knop" value="Nieuw">
                <input type="submit" name="delete" class="delete-knop" value="Verwijderen" style="margin-left: 5px;">
            </div>
        ...
        <div class="projecten-overzicht">
            ...
            <table cellspacing="0" class="order-table table">
                ...
                foreach ($data as $rows){
        	        ...
        	        <tr>
    	                <td><input name="project" type="checkbox" value="<?php echo $project_id; ?>" style="display: block" /></td>
    	                <td><a href="projecten/<?php echo $projectslug; ?>"><?php echo $projectnaam; ?></a></td>
    	                <td><?php echo $last_update; ?></td>
